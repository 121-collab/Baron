...
> 说明：这是把你在服务器上拆成 4 个脚本  
> `01_build_seurat.R`, `02_fig1_markers.R`, `03_fig2_corr.R`, `04_fig3_ductal_fig4_beta_PCA.R`  
> 合并后的**单文件版本**。  
>
> - 可以直接保存为 `Baron2016_pancreas_pipeline.R` 放到 GitHub。  
> - 默认假设 Baron 原始 `GSE84133_RAW` 解压目录结构与你服务器一致，只需要修改最上面的 `BASE_DIR`。  
> - 如果已经在服务器上跑完并生成 `Human_seurat_celltyped.rds` / `Mouse_seurat_celltype.rds`，  
>   重新运行时会**直接读取现成 RDS**，不会重复做 QC。  
